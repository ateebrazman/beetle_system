document.addEventListener('DOMContentLoaded', () => {
    ...
    // Cache elements and page layout parameters to avoid layout thrashing
    ...
    let activeProjectRect = null;

    document.querySelectorAll('.project-item').forEach(item => {
        item.addEventListener('mouseenter', () => {
            activeProjectEl = item;
            activeProjectRect = item.getBoundingClientRect();
        });
    ...
    document.addEventListener('mousemove', (e) => {
        ...
        if (activeProjectEl && activeProjectRect) {
            const img = activeProjectEl.querySelector('img');
            if (img) {
                const xRel = (mouseX - activeProjectRect.left) / activeProjectRect.width - 0.5;
                const yRel = (mouseY - activeProjectRect.top) / activeProjectRect.height - 0.5;
                
                // Subtle movement: shift image by 40px max
                gsap.to(img, {
                    x: xRel * 40,
                    y: yRel * 40,
                    duration: 0.6,
    ...
    function updateOnScroll(e) {
        ...
        // Scroll progress dimensions
        const totalHeight = pageScrollHeight - viewportHeight;
        const scrollPercent = totalHeight > 0 ? (scrollY / totalHeight) * 100 : 0;

        // === WRITE PHASE ===
        
        // 1. Curtains Reveal & Scroll Indicator Fade
        if (curtainTop && curtainBottom) {
            const progress = Math.min(scrollY / (viewportHeight * scrollFactor), 1);
        ...
        // 4. Image Parallax Writes
        parallaxData.forEach(data => {
            if (data.element) {
                data.element.style.transform = `scale(1.1) translateY(${data.move}px)`;
            }
        });

        // 5. Scroll Progress Write
        if (scrollProgress) {
            scrollProgress.style.width = scrollPercent + "%";
        }

        // 6. Back to Top Visibility Write
        if (backToTop) {
    ...
    if (cycleTrack && cWrapper) {
        // Wait for layout so we can measure and clone
        requestAnimationFrame(() => {
            requestAnimationFrame(() => {
                ...
                const advance = () => {
                    ...
                    // Re-measure height just in case before animating to prevent scaling shift
                    adjustTickerHeight();
        ...
        // Re-measure and adjust on window load and resize
        window.addEventListener('load', adjustTickerHeight);
        window.addEventListener('resize', adjustTickerHeight);
    }
    ...
    window.addEventListener('load', () => {
        // Recache after full asset loading
        cacheParallaxImages();
        pageScrollHeight = document.documentElement.scrollHeight;
        ...
        if (loader) {
            setTimeout(() => {
                ...
                // Immediately show main layout to cross-fade under fading loader
            ...
            }, 500); // Brief initial display
        } else {
            // No loader? Just reveal immediately
            document.body.classList.add('page-loaded');
            if (hero) hero.classList.add('reveal-active');
            setTimeout(triggerCookieConsent, 1500);
        }
    });
    ...
    if (reviewForm) {
        reviewForm.addEventListener('submit', async (e) => {
            ...
            formData.append('csrf_token', window.beetleCMS.csrfToken);
    ...
    if (mainContactForm) {
        mainContactForm.addEventListener('submit', async (e) => {
            ...
            // Append service and budget if present
            const serviceVal = document.getElementById('contact-service')?.value;
            const budgetVal = document.getElementById('contact-budget')?.value;
            if (serviceVal) formData.append('service', serviceVal);
            if (budgetVal) formData.append('budget', budgetVal);
            ...
            try {
                ...
                if (result.status === 'success') {
                    ...
                    // Reset selector chips to default active state
                    document.querySelectorAll('.interactive-selector').forEach(sel => {
                        const firstChip = sel.querySelector('.chip');
                        if (firstChip) {
                            sel.querySelectorAll('.chip').forEach(c => c.classList.remove('active'));
                            firstChip.classList.add('active');
                            const input = sel.querySelector('input[type="hidden"]');
                            if (input) input.value = firstChip.dataset.value;
                        }
                    });
                } else {
    ...
    // --- Contact Page Selector Chips Handling ---
    const setupChips = (containerId, hiddenInputId) => {
        ...
        if (container && hiddenInput) {
            ...
            chips.forEach(chip => {
                chip.addEventListener('click', () => {
                    ...
                    // If service chips, update subject too
                    if (hiddenInputId === 'contact-service') {
                        const subjectInput = document.getElementById('contact-subject');
                        if (subjectInput) subjectInput.value = chip.dataset.value;
                    }
                });
            });
        }
    };
    setupChips('service-chips-container', 'contact-service');
    setupChips('budget-chips-container', 'contact-budget');
    ...
    const tiltContainers = document.querySelectorAll('.img-reveal-wrapper, .project-img, .contact-card-glass, .studio-media-frame');

    tiltContainers.forEach(container => {
        const target = container.querySelector('img') || container;
        const isCard = container.classList.contains('contact-card-glass');
        if (!target) return;
        ...
        container.addEventListener('mousemove', (e) => {
            ...
            // Set CSS variables for spotlight highlight tracking
            ...
            gsap.to(target, {
                duration: 0.4,
                x: isCard ? 0 : deltaX * 30, // Parallax slide (disabled for cards)
                y: isCard ? 0 : deltaY * 30,
    ...
    // Cookie banner triggers are now handled programmatically after load completion
    ...
    if (projectItems.length > 0 && floatingPortal) {
        ...
        // Cache portal dimensions to prevent layout thrashing
        let portalWidth = 300;
        ...
        projectItems.forEach(item => {
            item.addEventListener('mouseenter', () => {
                ...
                // Query dimensions only once on activation
                portalWidth = floatingPortal.offsetWidth || 300;
                portalHeight = floatingPortal.offsetHeight || 300;
        ...
        const updatePortal = () => {
            ...
            // Offset the portal center using cached dimensions
            const offsetX = portalWidth / 2;
            const offsetY = portalHeight / 2;
    ...
    if (ctaPortal && ctaSpotlight) {
        ...
        ctaPortal.addEventListener('mouseleave', () => {
            ctaRect = null;
        });
        ctaPortal.addEventListener('mousemove', (e) => {
            ...
            const x = e.clientX - ctaRect.left;
            const y = e.clientY - ctaRect.top;
